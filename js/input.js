...
canvas.addEventListener('pointerdown', (e) => {
    ...
    // Team selection buttons - FIXED for scroll offset
    if (window._teamSelectBtns) {
        const scrollOffset = window._teamScrollOffset || 0;
        for (let btn of window._teamSelectBtns) {
            // The button's y position in the array is the ORIGINAL position (without scroll)
            // We need to check if pos.y is within the button's visible area
            // The button's visible y = btn.y - scrollOffset
    ...
    // Check for NEXT ROUND button (qualification)
    ...
    if (nextRoundBtn && pos.x >= nextRoundBtn.x && pos.x <= nextRoundBtn.x + nextRoundBtn.w && 
        ...
        SoundManager.playSFX('confirm');
        // Go to knockout bracket
        currentState = 'TOURNAMENT_BRACKET';
        // Simulate any remaining knockout matches that don't involve the player
        TournamentManager.simulateKnockoutRound();
        updateTouchUI();
        return;
    }
    
    // Check for OUT button
    ...
    if (nextBtn && pos.x >= nextBtn.x && pos.x <= nextBtn.x + nextBtn.w && 
        pos.y >= nextBtn.y && pos.y <= nextBtn.y + nextBtn.h) {
        SoundManager.playSFX('menuClick');
        if (TournamentManager.isComplete()) {
            currentState = 'TOURNAMENT_CHAMPION';
        } else {
